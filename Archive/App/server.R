...
DCF_Valuation <- function(Get, ticker, ExpectedGrowthRate, MarginSafety, GrowthDeclineRate,
        ...
        if (Get == "Plot_1"){
                ...
                temp_df <- data.frame(cbind("Ticker"=as.character(DCF_finaldf2$Ticker), "DCF_Valuation"=as.numeric(as.character(DCF_finaldf2$DCF_Valuation)), 
...
PE_Valuation <- function(Get, ticker, ExpectedGrowthRate, MarginSafety, DiscountRate){
        ...
        for (i in 1:length(ticker)){
                ...
                CurrentYearValue <- as.vector(((as.numeric(temp.df$Date[1])) * coefficients(linearModel)[2]) + coefficients(linearModel)[1])
...
shinyServer(
        function(input, output){
                ...
                output$DCF_Model.plot2 <- renderPlot({DCF_Valuation(Get = "Plot_2", ticker = input$ticker, ExpectedGrowthRate = input$ExpectedGrowthRate, MarginSafety = input$MarginSafety,
                                                                        GrowthDeclineRate = input$GrowthDeclineRate, DiscountRate = input$DiscountRate, DCF_YearsProjection = input$DCF_YearsProjection,
                                                                        LongTermGrowthRate = input$LongTermGrowthRate)})
                
                output$DCF_Model.table <- renderDataTable({DCF_Valuation(Get = "Table_1", ticker = input$ticker, ExpectedGrowthRate = input$ExpectedGrowthRate, MarginSafety = input$MarginSafety,
                                                                                      GrowthDeclineRate = input$GrowthDeclineRate, DiscountRate = input$DiscountRate, DCF_YearsProjection = input$DCF_YearsProjection,
                                                                                      LongTermGrowthRate = input$LongTermGrowthRate)})
                
                output$DCF_Model.table2 <- renderDataTable({DCF_Valuation(Get = "Table_2", ticker = input$ticker, ExpectedGrowthRate = input$ExpectedGrowthRate, MarginSafety = input$MarginSafety,
                                                                         GrowthDeclineRate = input$GrowthDeclineRate, DiscountRate = input$DiscountRate, DCF_YearsProjection = input$DCF_YearsProjection,
                                                                         LongTermGrowthRate = input$LongTermGrowthRate)})
                
                
                ##Group these output for PE Valuation Model Tab
                output$PE_Model.plot <- renderPlot({PE_Valuation(Get = "Plot_1", ticker = input$ticker, ExpectedGrowthRate = input$ExpectedGrowthRate, 
                                                                 MarginSafety = input$MarginSafety, DiscountRate = input$MarginSafety)})
                
                output$PE_Model.plot2 <- renderPlot({PE_Valuation(Get = "Plot_2", ticker = input$ticker, ExpectedGrowthRate = input$ExpectedGrowthRate, 
                                                                  MarginSafety = input$MarginSafety, DiscountRate = input$MarginSafety)})
                
                output$PE_Model.table <- renderDataTable({PE_Valuation(Get = "Table_1", ticker = input$ticker, ExpectedGrowthRate = input$ExpectedGrowthRate, 
                                                                       MarginSafety = input$MarginSafety, DiscountRate = input$MarginSafety)})
                
                output$PE_Model.table2 <- renderDataTable({PE_Valuation(Get = "Table_2", ticker = input$ticker, ExpectedGrowthRate = input$ExpectedGrowthRate, 
                                                                        MarginSafety = input$MarginSafety, DiscountRate = input$MarginSafety)})
                ...
                output$Cash_Equivalents.table <- renderDataTable({Misc_Metrics(Get = "CashEquivalents", Get2 = "Table", ticker = input$ticker)})
